...
async function loadAdminFinanzas() {
    try {
        ...
        if (!tbody) {
            console.error("No se encontró el elemento adm-payments-table-body");
            return;
        }
        tbody.innerHTML = "";
        
        if (!data.pagos || data.pagos.length === 0) {
            tbody.innerHTML = `<tr><td colspan="7" class="p-4 text-center text-xs text-outline">No hay pagos registrados.</td></tr>`;
            return;
        }
        ...
        data.pagos.forEach(p => {
            const statusClass = {
                ...
                "Pendiente": "status-pendiente",
                "En Revision": "status-revision",
                "Verificado": "status-pagado",
                "Rechazado": "status-pendiente"
            }[p.estado] || "status-pendiente";
            
            // Construir acciones
            let actionsHtml = '';
            
            // Botones de aprobar/rechazar (solo si está en revisión)
            if (p.estado === "En Revision" || p.estado === "Verificado") {
                actionsHtml = `
                    <div class="flex flex-col gap-1 items-center">
                        <div class="flex gap-1">
                            <button class="bg-green-600 text-white px-2 py-0.5 rounded text-[10px] font-bold hover:opacity-90" onclick="validatePayment(${p.id}, 'Pagado')">✓ Aprobar</button>
                            <button class="bg-red-600 text-white px-2 py-0.5 rounded text-[10px] font-bold hover:opacity-90" onclick="validatePayment(${p.id}, 'Rechazado')">✗ Rechazar</button>
                        </div>
                        ${p.comprobante_url ? `<button class="border border-outline px-1.5 py-0.5 rounded hover:bg-surface-container text-[10px]" onclick="viewReceiptImage('${p.comprobante_url}', ${p.monto}, '${p.referencia}', '${p.metodo}', '${p.concepto}')">📄 Ver</button>` : ''}
                    </div>
                `;
            } else {
                actionsHtml = `<span class="text-xs text-outline">✅ Conciliado</span>`;
            }
            
            // Botón de verificación bancaria (solo si está en revisión o pendiente)
            const mostrarVerificacion = (p.estado === "En Revision" || p.estado === "Pendiente");
            const botonVerificacion = mostrarVerificacion ? `
                <button class="bg-blue-600 text-white px-2 py-0.5 rounded text-[10px] font-bold hover:opacity-90 w-full mt-1" 
                        onclick="verificarPagoBanco(${p.id}, '${p.referencia}', ${p.monto})">
                    🏦 Verificar Banco
                </button>
            ` : '';
            ...
            if (p.proveedor_nombre) {
                proveedorHtml = `
                    <div class="text-xs">
                        <span class="font-bold text-primary">${p.proveedor_nombre}</span>
                        <br>
                        <span class="text-[9px] text-outline">${p.proveedor_cuenta || 'Sin cuenta'}</span>
                        <br>
                        <span class="text-[9px] text-outline">${p.proveedor_banco || ''}</span>
                    </div>
                `;
            }
            ...
            tr.innerHTML = `
                <td class="px-3 py-3 font-semibold text-sm">${p.numero_apartamento || 'N/A'}</td>
                <td class="px-3 py-3 text-sm">${p.concepto || 'N/A'}</td>
                <td class="px-3 py-3">${proveedorHtml}</td>
                <td class="px-3 py-3 text-xs text-outline">${p.metodo || 'N/A'}<br><span class="font-mono">${p.referencia || 'N/A'}</span></td>
                <td class="px-3 py-3 text-right font-bold text-sm">$ ${(p.monto || 0).toFixed(2)}</td>
                <td class="px-3 py-3 text-center">
                    <span class="status-pill ${statusClass}">${p.estado || 'N/A'}</span>
                </td>
                <td class="px-3 py-3 text-center">
                    <div class="flex flex-col gap-1 items-center">
                        ${actionsHtml}
        ...
        console.error("Error en loadAdminFinanzas:", err);
        const tbody = document.getElementById("adm-payments-table-body");
        if (tbody) {
            tbody.innerHTML = `<tr><td colspan="7" class="p-4 text-center text-xs text-error">Error al cargar los datos: ${err.message}</td></tr>`;
        }
    }
}
...
// ============================================================
// NUEVAS FUNCIONES AGREGADAS: PROVEEDORES Y VERIFICACIÓN BANCARIA
// ============================================================

async function verificarPagoBanco(pagoId, referencia, monto) {
    if (!confirm(`¿Confirmar verificación bancaria del pago ${referencia} por $${monto}?`)) return;
    
    try {
        const res = await fetch("/api/v1/finanzas/verificar_pago", {
            method: "POST",
            headers: { 
                "Content-Type": "application/json",
                "Authorization": `Bearer ${jwtToken}`
            },
            body: JSON.stringify({ pago_id: pagoId, referencia, monto })
        });
        const data = await res.json();
        ...
        // Recargar la vista de finanzas
        await loadAdminFinanzas();
    } catch (err) {
        alert("❌ Error al verificar con el banco: " + err.message);
    }
}
