...
def get_cuotas(current_user):
    ...
    if current_user["rol"] == "Residente":
        ...
        pagos = conn.execute("""
            SELECT p.*, pr.nombre as proveedor_nombre, pr.cuenta_bancaria as proveedor_cuenta, pr.banco as proveedor_banco
            FROM pagos p 
        ...
        """).fetchall()
        pagos = conn.execute("""
            SELECT p.*, u.numero_apartamento, pr.nombre as proveedor_nombre, 
                   pr.cuenta_bancaria as proveedor_cuenta, pr.banco as proveedor_banco,
                   vb.estado as verificacion_estado, vb.fecha_verificacion, vb.mensaje as verificacion_mensaje
            FROM pagos p 
            JOIN unidades u ON p.unidad_id = u.id 
            LEFT JOIN proveedores pr ON p.proveedor_id = pr.id
            LEFT JOIN verificaciones_bancarias vb ON p.id = vb.pago_id
            ORDER BY p.id DESC
        """).fetchall()
        conn.close()
...
def reportar_pago(current_user):
    ...
    # If proveedor_id is provided, validate it exists
    if proveedor_id:
        proveedor = conn.execute("SELECT id FROM proveedores WHERE id = ?", (proveedor_id,)).fetchone()
        if not proveedor:
            conn.close()
    ...
    conn.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, 'En Revision', ?, ?)
    """, (unit["id"], float(monto), now_date, referencia, metodo, concepto, filename, proveedor_id))
...
def validar_pago(current_user):
    ...
    if pago["estado"] != "En Revision" and pago["estado"] != "Verificado":
        conn.close()
...
def get_egresos(current_user):
    ...
    egresos = conn.execute("""
        SELECT e.*, pr.nombre as proveedor_nombre, pr.cuenta_bancaria as proveedor_cuenta, pr.banco as proveedor_banco
        FROM egresos e
...
def create_egreso(current_user):
    ...
    conn.execute("INSERT INTO egresos (concepto, monto, fecha, soporte_pdf, proveedor_id) VALUES (?, ?, ?, NULL, ?)", 
                 (concepto, float(monto), now_date, proveedor_id))
    conn.commit()
    conn.close()
...
def get_db_table(current_user):
    ...
    allowed_tables = ['usuarios', 'unidades', 'pagos', 'egresos', 'accesos', 'telemetria', 'configuracion_sistema', 'bitacora', 'visitantes', 'proveedores', 'verificaciones_bancarias']
...
# ============================================================
# NUEVAS RUTAS PARA PROVEEDORES Y VERIFICACION BANCARIA
# ============================================================
...
def create_proveedor(current_user):
    ...
    if not nombre or not tipo_servicio or not cuenta_bancaria or not banco:
        return jsonify({"mensaje": "Nombre, tipo de servicio, cuenta bancaria y banco son requeridos"}), 400
    ...
    try:
        conn.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (nombre, tipo_servicio, cuenta_bancaria, banco, rif, telefono, email, contacto_nombre, ahora))
        conn.commit()
        
        log_event(current_user["id"], "Registro de proveedor", f"Se registro proveedor {nombre}")
        return jsonify({"mensaje": "Proveedor registrado exitosamente"}), 201
    except sqlite3.IntegrityError:
        return jsonify({"mensaje": "Error al registrar proveedor"}), 400
    finally:
        conn.close()

# Verificar pago con el banco (simulación)
@app.route("/api/v1/finanzas/verificar_pago", methods=["POST"])
@token_required
@roles_allowed("Administrador")
def verificar_pago_banco(current_user):
    data = request.get_json() or {}
    pago_id = data.get("pago_id")
    referencia = data.get("referencia")
    monto = data.get("monto")
    
    if not pago_id or not referencia or not monto:
        return jsonify({"mensaje": "pago_id, referencia y monto son requeridos"}), 400
    ...
    tiempo_espera = random.uniform(0.8, 2.5)
    time.sleep(tiempo_espera)
    
    # 80% de éxito en la verificación
    exitoso = random.random() < 0.8
    ahora = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    
    if exitoso:
        mensaje = "Pago verificado con el banco exitosamente"
        estado_verificacion = "exitoso"
        respuesta_banco = json.dumps({
            "codigo": "00",
            "mensaje": "Transaccion aprobada",
            "referencia": referencia,
            "monto_confirmado": float(monto)
        })
        conn.execute("UPDATE pagos SET estado = 'Verificado', fecha_verificacion = ? WHERE id = ?", (ahora, pago_id))
    else:
        mensaje = "No se encontró la transacción en el banco. Verifique los datos."
        estado_verificacion = "fallido"
        respuesta_banco = json.dumps({
            "codigo": "98",
            "mensaje": "Transaccion no encontrada",
            "referencia": referencia
        })
        conn.execute("UPDATE pagos SET estado = 'Rechazado', fecha_verificacion = ? WHERE id = ?", (ahora, pago_id))
    ...
    conn.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
    """, (pago_id, referencia, float(monto), ahora, estado_verificacion, mensaje, respuesta_banco))
    
    conn.commit()
    conn.close()
    
    log_event(current_user["id"], f"Verificacion bancaria: {estado_verificacion}", 
              f"Pago {referencia} - {mensaje}")
    ...
    return jsonify({
        ...
        "referencia": referencia,
        "monto_verificado": float(monto) if exitoso else None,
        "estado": estado_verificacion
    })

# Obtener historial de verificaciones bancarias
@app.route("/api/v1/finanzas/verificaciones", methods=["GET"])
@token_required
@roles_allowed("Administrador")
def get_verificaciones(current_user):
    conn = get_db_connection()
    verificaciones = conn.execute("""
        SELECT vb.*, p.referencia as pago_referencia, p.monto as pago_monto, 
               u.numero_apartamento, us.nombre as usuario_nombre
        FROM verificaciones_bancarias vb
        JOIN pagos p ON vb.pago_id = p.id
        JOIN unidades u ON p.unidad_id = u.id
        LEFT JOIN usuarios us ON u.residente_id = us.id
        ORDER BY vb.id DESC
        LIMIT 100
    """).fetchall()
    conn.close()
    return jsonify([dict(v) for v in verificaciones])
